(function () {
  ...
  const defaultGeoLabel = document.getElementById("default-geo-label");
  const effectiveGeo = document.getElementById("effective-geo");

  async function refresh() {
    const res = await fetch(API);
    if (!res.ok) {
      noteEl.textContent = "Could not load settings (" + res.status + ").";
      return;
    }
    ...
    defaultGeoLabel.textContent = data.linkedin_default_geo_id || "103644278";
    effectiveGeo.textContent = data.linkedin_effective_geo_id || "—";
    noteEl.textContent = data.save_note || "";
  }
